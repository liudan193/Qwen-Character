...
class Tracking:
    ...
    supported_backend = ["wandb", "mlflow", "swanlab", "vemlp_wandb", "tensorboard", "console", "clearml", "hope_tracking"]
    ...
    def __init__(self, project_name, experiment_name, default_backend: Union[str, List[str]] = "console", config=None):
        ...
        if "swanlab" in default_backend:
            ...
            SWANLAB_HOST = os.environ.get("SWANLAB_HOST", None)
            if SWANLAB_API_KEY:
                swanlab.login(SWANLAB_API_KEY, host=SWANLAB_HOST)  # NOTE: previous login information will be overwritten
...
class _HopeTrackingAdapter:
    ...
    def __init__(self):
        print("Hope Tracking Enabled!")

    def log(self, data, step):
        from hope import tracking
        for key, value in data.items():
            tracking.metric(key, value, step)
