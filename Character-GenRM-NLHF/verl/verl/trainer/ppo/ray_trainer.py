...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.use_distributed_reward_fn = Role.RewardManager in role_worker_mapping # yanyuchen04 use distributed func
    ...
    def _write_eval_file(self, inputs, outputs, scores, gt, source):
        local_global_step_folder = os.path.join(self.config.trainer.default_local_dir,
                                                f'global_step_{self.global_steps}')
        eval_file_file = os.path.join(local_global_step_folder, 'evaluation.jsonl')
        with jsonlines.open(eval_file_file, 'w') as writer:
            for input, output, score, _gt, _source in zip(inputs, outputs, scores, gt, source):
                writer.write({
                    "source": _source, "input": input, "gt": _gt, "output": output, "score": score
                })
    ...
    def init_workers(self):
        ...
        if self.use_distributed_reward_fn:
            resource_pool = self.resource_pool_manager.get_resource_pool(Role.RewardManager)
            rm_manager_cls = RayClassWithInitArgs(self.role_worker_mapping[Role.RewardManager], tokenizer=self.config.actor_rollout_ref.model.path)
            self.resource_pool_to_cls[resource_pool]['rm_func'] = rm_manager_cls
        ...
        if self.use_distributed_reward_fn:
            self.rm_func = all_wg['rm_func']
            self.rm_func.init_model()
            self.reward_fn = self.rm_func.remote_call
            self.val_reward_fn = self.rm_func.remote_call
    ...
    def _save_checkpoint(self):
        ...
        # transfer to hf weights
        subprocess.Popen(['python3', 'scripts/model_merger.py', '--local_dir', actor_local_path])
