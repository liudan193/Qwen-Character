...
EXP_NAME="exp-ds_r1_distill_qwen_7b-rm_nlhf"
...
train_files="['/path/to/rm_nlhf_train_rl_helpsteer3_tuluif.parquet']"
...
python3 -m recipe.rm_nlhf.main_ppo \
  ...
  reward_model.reward_manager=rm_nlhf \
  reward_model.reward_kwargs.weight_rule=1.0 \
  reward_model.reward_kwargs.weight_model=0.5 \
  reward_model.enable=True \
  reward_model.is_update=True \
  reward_model.reward_scale=False \
  reward_model.model.path=${INIT_CKPT_REWARD_MODEL} \
  ...
  reward_model.model.enable_activation_offload=False \
  reward_model.optim.lr=5e-6 \
  reward_model.optim.lr_warmup_steps_ratio=0.0 \
  reward_model.ppo_epochs=1 \
  reward_model.ppo_micro_batch_size_per_gpu=2 \
  reward_model.launch_reward_fn_async=False \
  reward_model.collaborate.data_filter_for_rm_update=False \
